...
setwd("/rds/project/rjh234/rds-rjh234-mrc-epid/Studies/People/Maik/compare_SomaLogic_Olink/09_coloc_cis_phewas/")
...
res.soma        <- paste0("cat /home/mdp50/rds/rds-rjh234-mrc-epid/Studies/People/Maik/compare_SomaLogic_Olink/06_SCALLOP_CVD1/sum_stats/",
                          soma,".txt",
                          ## add chromosome and position
                          " | awk -F'[r:_]' -v chr=", chr.s, " -v low=", pos.s, " -v upp=", pos.e, 
                          " '$2 == chr && $3 >= low && $3 <= upp {print $0}' -")
res.soma        <- data.table::fread(cmd = res.soma, sep = "\t", header = F, data.table = F)
## add names
names(res.soma) <- c("MarkerName", "Allele1", "Allele2", "Freq1", "FreqSE", "Effect", "StdErr", "Pvalue", "Direction", "TotalSampleSize")

## add chromosoma bad position
res.soma$chr    <- as.numeric(sapply(res.soma$MarkerName, function(x) strsplit(x, "r|:")[[1]][2]))
res.soma$pos    <- as.numeric(sapply(res.soma$MarkerName, function(x) strsplit(x, ":|_")[[1]][2]))
...
#-----------------------------------------#
##--      subset SCALLOP results       --##
#-----------------------------------------#

## subset the SCALLOP results
res.soma <- merge(res.soma, tmp.info[, c("MarkerName", "rsid")])
  ...
  if(nrow(tmp.phewas)>0){
    ...
    res.coloc <- lapply(1:nrow(phewas.results), function(x){
      ...
      if(naive.coloc$summary$PP.H4.abf > .7 | naive.coloc$summary$ld.check.top > .8){
        ...
        png(paste0("graphics_scallop//", soma, ".", phewas.results$id.ieu[x], ".", chr.s, ".", pos.s, ".", pos.e, ".png"), width=16, height=8, units="cm", res=100)
    ...
    ## write to file
    write.table(phewas.results, paste("output_scallop//results.phewas", soma, chr.s, pos.s, pos.e, "txt", sep="."), sep="\t", row.names=F)
    
    
  }else{
    cat("found no evidence for", soma, "on chr", chr.s, "between", pos.s, "and", pos.e, "\n")
    ## write to file
    write.table(data.frame(pheno=soma, id.ieu=NA), paste("output_scallop//results.phewas", soma, chr.s, pos.s, pos.e, "txt", sep="."), sep="\t", row.names=F)
  }
